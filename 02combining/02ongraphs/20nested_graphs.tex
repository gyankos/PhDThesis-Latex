...
Therefore, any object $o$ is identified by its id and is completely described by the functions $\ell$, $\xi$ and $\phi$. Consequently, the \index{General Semistructured Model|textbf} can be described as follows:
\[GSM=(o,O,\ell,\xi,\phi)\]
...
A general semistructured data model $GSM=(o,O,\ell,\xi,\phi)$ is said to be nesting loop free\GSMindex{nesting loop free} if each object $o\in O$ does not contain itself at any nesting level. More formally:
...
\end{numcases}
The former definition can be also used to sort the children $\varphi(o)$ of one single containing object $o$ via their relative height as follows\footnote{See line \ref{v3:ho}.}\index{$h_o$}:
...
Given two GSMs $n=(o,O,\ell,\xi,\phi)$ and $n'=(o',O',\ell',\xi',\phi')$, it is said that $n'$ is a \textbf{substructure}\GSMindex{substructure} of $n$ ($n'\subseteq n$) if and only if it
...
\begin{definition}[Set operations over lists]
	...
	where \texttt{distinct} returns the element where the repeated elements are preserved, and only the leftmost instance is kept\footnote{This operator can be also defined in \texttt{script}, which will be defined in the first subsection, as follows: \scriptline{distinct = x -> \{fold [x, y -> \{if (y[0] in y[1]) then y[1] else [y[0]] ++ y[1]\}]\}}}:
