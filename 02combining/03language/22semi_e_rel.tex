...
\[J_\otimes(\alpha,\texttt{bin}(p')\texttt{bin}(p'')xy,\omega,s)=\texttt{elect}_{\omega}\left(\bigotimes^{\texttt{bin}(p')\cdot\texttt{0}\cdot\texttt{bin}(p'')xy}_{u\in s}\texttt{elect}_u(\alpha)\right)\]
