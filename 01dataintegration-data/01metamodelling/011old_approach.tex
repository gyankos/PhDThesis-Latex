...
%Following this scenario, suppose that the local data warehouse provides a table : at this point, we have to grasp all the admission on a remote location, that we know that is called . In particular, the 
...
% refers to a machine readable representation allowing to categorize and classificate a patient's disease. Then, the two schemas associated to each table are extracted and compared, as  within the ontology field, t

The schema alignment process can be subsumed as follows:
\begin{alphalist}
	\item \label{lab:item:strua} each attribute having the same name will contain the same concept and, whether similar concepts are represented in different formats and representations, some transcoding function\index{transcoding|see{$\stigma$}} $\transcoding$ are associated \cite{GolfarelliMPRT12}. Otherwise, a correspondence is required:
...
\begin{figure}
    ...
    \begin{minipage}[t]{\textwidth}
      ...
      \subcaption{Representng the local clinical record using the schema in \cite{ManolescuFK01}.}
      \label{fig:ManolescuSchema}
    \end{minipage}
    \caption{Representating the schema associated to the XML files using the notation provided in \cite{BaaziziLCGS17} for JSON documents. Instead of associating values to each property, we associate properties to their types. Arrays of \texttt{T} elements are represented with the \texttt{[T*]} syntax, where \texttt{T} may represent another record, representing a composite type.}
...
\begin{figure}[!p]
	...
	\begin{minipage}[t]{\textwidth}
		...
		\subcaption{Complete alignment process of the two schemas associated to the XML files. Thre green lines remark the alignments between the schemas, the blue lines mark the detection of keys within a local schema. When these alignment edges do not align identities that appear on a different field, such id field is marked with a filled circle connected to those edges, The red question marks underline the fields that are missing in the other schema, and that will be filled with missing fields.}
...
\begin{figure}[!p]
	...
	\begin{minipage}[t]{\textwidth}
		...
		\subcaption{Representng the external clinical record using the schema in \cite{APoggi06}.}
		\label{fig:APoggiXML}
	\end{minipage}
...
\index{data cleaning}
\index{alignment}
Given the considerations of the former paragraph, we would like to use the outcomes of the data integration process presented in Section \ref{subsec:treunouno} as imputations for %first alignment steps \iffalse the first integration step \fi to associate both the ward information \iffalse to the XML integrated data coming from the previous phase (Figure \vref{fig:XMLDataMergeAfterAlignment}), \fi and the ICD-9-CM code from the Hospitalization table, in order to impute 
...
%With this example we want also to show that schema alignment could be also used to perform inputations, hence providing a data cleaning step.
%Hereby, the data integration phase could resemble the data cleaning one (e.g., imputation): such aspect will be covered by Section \ref{sec:indatabase}.
%In order to meet our goal, 
We must now use both the source data (Figure \vref{tab:Admissions}) and the transcoding functions generated from the alignment between the two schemas (Figure \vref{fig:relalign}). Firstly, we still have to create correspondences between each XML \texttt{treatment\_entry} and a record belonging to either \texttt{Admissions} or \texttt{Hospitalization}.%, in order to check from which data source has to be integrated. 
...
Unstructured data represents data that, even if represented in a digital format, could not be  immediately used as pieces of information %\iffalse . According to the DIKW pyramid formulation \cite{DIKW}, data is information \fi which is of no use
 until its transformation in a semantic dependant form is provided \cite{OrgKno}: such data has usually no associated description of how to interpret such information. Figure \ref{fig:unstructuredmedical} provides some examples of unstructured contents: the index at bottom left represents an association between some medical terms represented in a hierarchical form and some ICD-9-CM codes, while the picture represents a tumour of the salivary glans. As we will see in Section \vref{sec:unstructured}, full text content shall be represented either in a syntactical structured form (\textit{spans}) or semistructured with graph (\textit{universal dependencies graph})\index{graph!dependency graph} or nested graph\index{graph!nested graph}\footnote{Since both nested graph data integration is proposed for the first time by this thesis, we refer to Section \vref{sss:ngdi} for further details.} notation (proposed by this thesis in Section \vref{sec:semistructunstradata}). Suppose now that we want to integrate the three textual representations altogether: as we can see, we cannot mix them at the text level, because  full text documents could be represented in different formats (txt, doc, RDF, wiki markup or pdf) and may also express different concepts which require to be differently interpretated. As a consequence, the best way to integrate unstructured data is to first provide them some structure reflecting their semantics, and \iffalse themerge these to types of documents is to extract their structured and semistructured representation, and \fi only then try to merge the data.
\begin{figure}
	\centering
...
\begin{figure}
	...
	\begin{minipage}[t]{\textwidth}
		...
		\subcaption{Representation of the taxonomy as a graph, where the association between the ICD-9-CM codes is made explicit..}
...
%Such representations require some further preprosessing to transform it into n-ary relations
%%%full text in order to provide a machine readable representation. Such representation, in any case, requires some pre-processing to transform it into a hypergraph, thus allowing to represent edges having more than one single source vertex and more than one target vertex, and hence as $n$-ary relations. This relational representation is already
%described in \textbf{IBM Watson} documentation \cite{IBMWatson} for textual knowledge representation. 
%%This thesis also provides an alternative representation for full-text corpora, called ``nested graphs'', allowing  to represent semantical concepts both in a human readable and a machine readable representation. 


\subsection{Alignign (Nested) Graphs}\label{sss:ngdi}
\index{graph!nested graph}
If we shift the schema alignment process from (semi)structured data to graphs, it
%As showed by the following example, through this representation 
...
%first associate concepts to the graph and then perform alignments on the actual data, eventually using graph matching approximating techniques \cite{VirgilioMT15}.
...
\begin{figure}[!p]
	...
	\begin{minipage}[t]{\textwidth}
		...
		\label{fig:answer2}
	\end{minipage}
	\caption{Nested graph representation of both a question (\subref{fig:question1}) and two possible candidate answers (\subref{fig:answer1} and \subref{fig:answer2}), where the last one is correct.}
\end{figure}
	...
	%For further reference, see Section \vref{sec:fullasgraph}, where the graph rewriting language is going to be firstly addressed. 
