...
my matrix/.style={
	...
	\begin{scope}[
		...
		\node at (0,0) {$ G_1$};
		 % Agents:
		\node  (w1) {};
		\node [place] (w2) [right of=w1] {$v_0$};
		\node [place] (w3) [above of=w1] {$v_2$};
		\node [place] (w4) [above of=w2] {$v_1$};
		\draw[-latex] (w3) edge node [above] {$e_o$} (w4);
		\draw[-latex] (w4) edge [bend right] node [right] {$e_1$} (w2);
	\end{scope}
